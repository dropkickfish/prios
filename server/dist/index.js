...
function isTimeAllowed(startMs, endMs, schedule) {
    if (!schedule)
        return { allowed: true };
...
async function refreshGoogleToken() {
    const settings = await db.select().from(schema.appSettings).where(eq(schema.appSettings.id, 'singleton'));
    if (!settings[0]?.googleRefreshToken)
        return null;
    ...
    if (!email) {
        try {
            ...
            if (restRes.ok) {
                ...
                email = list.items?.find((i) => i.primary)?.id;
                if (email) {
...
async function createCalendarEvent(title, startTime, durationMinutes) {
    const accessToken = await refreshGoogleToken();
    const settings = await db.select().from(schema.appSettings).where(eq(schema.appSettings.id, 'singleton'));
    if (!accessToken || !settings[0]) {
...
fastify.post('/api/boards', async (request) => {
    const { name, availabilitySchedule, colour } = request.body;
    // Get max order
...
fastify.get('/api/boards/:boardId/cards', async (request) => {
    ...
    if (allCardIds.length > 0) {
        ...
        cardTagsJoined.forEach(ct => {
            if (!tagsMap.has(ct.cardId))
                tagsMap.set(ct.cardId, []);
...
fastify.put('/api/boards/reorder', async (request) => {
    const { boards } = request.body; // Array of { id, order }
...
fastify.post('/api/cards/:id/schedule', async (request, reply) => {
    const { id } = request.params;
    const { scheduledAt, durationMinutes } = request.body;
    const cardResult = await db.select().from(schema.cards).where(eq(schema.cards.id, id));
    if (!cardResult[0])
        return reply.status(404).send({ error: 'Card not found' });
    const card = cardResult[0];
    ...
    if (scheduledAt) {
        ...
        const conflict = checkEvents.find((e) => {
...
fastify.delete('/api/boards/:id', async (request) => {
    ...
    if (cardIds.length > 0) {
        ...
        await db.delete(schema.dependencies).where(or(inArray(schema.dependencies.blockingCardId, cardIds), inArray(schema.dependencies.blockedCardId, cardIds)));
        // 3. Delete cardTags
...
fastify.post('/api/tags', async (request) => {
    const { name, colour, boardId } = request.body;
    if (!boardId)
        throw new Error('boardId is required');
    const lowerName = name.toLowerCase().trim();
    // Check for existing tag in this board
    const existing = await db.select().from(schema.tags).where(and(eq(schema.tags.boardId, boardId), eq(schema.tags.name, lowerName)));
    if (existing[0])
        return existing[0];
...
fastify.post('/api/cards/:cardId/tags', async (request) => {
    ...
    const existing = await db.select().from(schema.cardTags).where(and(eq(schema.cardTags.cardId, cardId), eq(schema.cardTags.tagId, tagId)));
    if (existing[0])
        return existing[0];
...
fastify.delete('/api/cards/:cardId/tags/:tagId', async (request) => {
    ...
    await db.delete(schema.cardTags).where(and(eq(schema.cardTags.cardId, cardId), eq(schema.cardTags.tagId, tagId)));
    return { success: true };
});
// Stats Endpoints
...
fastify.post('/api/calendar/sync', async (request) => {
    ...
    const scheduledCards = await db.select()
        .from(schema.cards)
        .innerJoin(schema.statuses, eq(schema.cards.statusId, schema.statuses.id))
        .where(and(eq(schema.statuses.category, 'scheduled'), isNotNull(schema.cards.externalEventId) // Is Not Null check
    ));
    if (scheduledCards.length === 0)
        return { synced: 0, moved: 0, deleted: 0 };
    ...
    for (const card of scheduledCards) {
        if (!card.cards.externalEventId)
            continue;
        // Find matching event
        const event = events.find((e) => e.uid === card.cards.externalEventId || (e.uid && card.cards.externalEventId && e.uid.includes(card.cards.externalEventId)) || (card.cards.externalEventId && e.uid && card.cards.externalEventId.includes(e.uid)));
        if (!event) {
